...
RESULTS_PATH = "results/tree_meteo_more_model_set_scores.csv"
...
MODEL_SETS = {
    "all4": BASE_MODELS,
    "hist_only": {"hist": HIST},
    "all4_hist": {**BASE_MODELS, "hist": HIST},
    "lgbm_xgb_extra_hist": {k: BASE_MODELS[k] for k in ["lgbm", "xgb", "extra"]} | {"hist": HIST},
}
...
def evaluate_model_set(model_set_name, models, weather, labels, scada_by_group):
    ...
    for group, data in per_group.items():
        ...
        for variant, pred in {
            ...
            rows.append({"model_set": model_set_name, "variant": variant, "group": group, "score": score, "nmae": nmae, "ficr": ficr})
    return rows
