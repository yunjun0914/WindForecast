...
from utils.scada_direction import apply_direction_teacher, fit_direction_teacher


RESULTS_PATH = "results/tree_direction_teacher_fast_scores.csv"
FOLDS = [
...
def main():
    ...
    weather_base = add_meteo_block(build_weather_features(ldaps, gfs), meteo, "all_meteo")
    ...
    for fold in FOLDS:
        ...
        for group in GROUP_CAPACITY_KWH:
            teacher_cache[(group, "none")] = weather_base
            for kind in KINDS:
                if kind == "none":
                    continue
                teacher = fit_direction_teacher(weather_teacher, scada_by_group[group], group, fold["val_start"], kind)
                directed = apply_direction_teacher(weather_teacher, teacher, group)
                direction_cols = ["forecast_kst_dtm"] + [c for c in directed.columns if c.startswith(f"{group}_scada_wd_")]
                teacher_cache[(group, kind)] = weather_base.merge(directed[direction_cols], on="forecast_kst_dtm", how="left")
        ...
        for kind in KINDS:
            ...
            for group, capacity in GROUP_CAPACITY_KWH.items():
                ...
                if curve is None:
                    continue
                weather = add_power_curve_feature(teacher_cache[(group, kind)], HUB_HEIGHT_PROXY_COL, curve, GROUP_N_TURBINES[group])
                ...
                rows.append({**fold, "direction_teacher": kind, "group": group, "score": score, "nmae": nmae, "ficr": ficr, "n": len(y_val)})
                group_scores.append(score)
            if group_scores:
                mean_score = sum(group_scores) / len(group_scores)
                rows.append({**fold, "direction_teacher": kind, "group": "mean", "score": mean_score, "nmae": None, "ficr": None, "n": None})
                print(f"{kind}: {mean_score:.4f}")

    results = pd.DataFrame(rows)
    results.to_csv(RESULTS_PATH, index=False, encoding="utf-8-sig")
    summary = (
        results[results["group"] == "mean"]
        .groupby(["fold_type", "direction_teacher"], as_index=False)
