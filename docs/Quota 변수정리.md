...
- TCN은 위 입력을 시계열 local panel로 사용하며 기존 FiCR-only loss와 그룹 target 직접 예측 구조를 유지한다.
- TREE는 같은 100/94개 입력을 표 형식으로 직접 사용하며 기존 그룹별 LGBM 하이퍼파라미터와 가중 정책을 유지한다.
- PINN은 같은 100/94개 입력을 outer-train 전용 RF OOB 풍속 teacher에 사용한다. PINN 본체의 공기밀도·달력 보조입력, 물리 제약 및 FiCR-only loss는 유지한다.
- 세 모델 모두 같은 고정 격자 계약을 사용하며 터빈별 출력을 만든 뒤 합산하지 않고 그룹 target을 직접 예측한다.
