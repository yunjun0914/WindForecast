...
Quota v2는 변수 공식을 바꾸지 않고 계산에 사용하는 NWP 공간 범위를 그룹 단위로 바꾼다.

1. 각 outer fold의 train 연도 SCADA 풍속만 사용한다.
2. 각 터빈에 대해 LDAPS와 GFS를 따로 평가하고 raw L3가 가장 낮은 `source/grid/level`을 선택한다.
3. 선택된 격자의 전체 NWP 열을 터빈 수만큼 모아 터빈 가중 패널을 만든다.
4. 패널 평균값을 그룹 중심 좌표의 `group_anchor`로 추가한다.
5. 기본값, 공간 통계, 축 투영, 물리 파생량 및 시간 파생량을 이 패널에서 다시 계산한다.
6. validation 연도는 격자 선택에 사용하지 않는다.
...
- v1 control: 그룹별 Quota v1 64개 + 터빈별 `wake_*` 및 fold-safe `optgrid_*`를 사용한다.
- v2: Quota v1 64개만 같은 개수의 group-local Quota v2로 교체한다.
- 두 버전 모두 G1/G2는 총 106개, G3는 총 99개 입력을 사용한다.
