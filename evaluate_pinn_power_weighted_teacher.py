...
RESULTS_PATH = "results/pinn_power_weighted_teacher_scores.csv"
VAL_START = "2024-01-01 01:00:00"
GROUP1 = "kpx_group_1"
GROUP2 = "kpx_group_2"
GROUP3 = "kpx_group_3"
BASE_GROUP_SCADA = {GROUP1: "vestas", GROUP2: "vestas", GROUP3: "unison"}
POWER_TARGETS = ["scada_ws_power_weighted", "scada_ws_power_weighted_cubic", "scada_power_active_ratio"]
TARGET_COLS = EXT_TARGETS + POWER_TARGETS


def build_power_weighted_targets(scada_df, group):
    scada = scada_df.copy()
    ...
    for hour, hourly in scada.groupby("hour"):
        ...
        active = power > 10.0
        if power.sum() > 1e-6:
            pw_mean = np.sum(ws * power) / np.sum(power)
            pw_cubic = np.cbrt(np.sum((np.clip(ws, 0, None) ** 3) * power) / np.sum(power))
        else:
            pw_mean = np.nanmean(ws)
            pw_cubic = np.cbrt(np.nanmean(np.clip(ws, 0, None) ** 3))
        rows.append(
            {
                "forecast_kst_dtm": hour,
                "scada_ws_power_weighted": pw_mean,
                "scada_ws_power_weighted_cubic": pw_cubic,
...
def build_targets(scada_df, group):
    base = build_extended_scada_targets(scada_df, group)
    power = build_power_weighted_targets(scada_df, group)
    out = base.merge(power, on="forecast_kst_dtm", how="inner")
    return out.dropna(subset=TARGET_COLS).reset_index(drop=True)
...
def fit_teacher(weather, scada_df, group, fit_before=VAL_START):
    targets = build_targets(scada_df, group)
...
def apply_teacher(weather, teacher, v_mode):
    ...
    for col in TARGET_COLS:
        out[col] = pred[col]
    if v_mode == "pw_mean":
        v = pred["scada_ws_power_weighted"]
    elif v_mode == "pw_cubic":
        v = pred["scada_ws_power_weighted_cubic"]
    elif v_mode == "mix_pw_cubic_p90":
        v = 0.5 * pred["scada_ws_power_weighted_cubic"] + 0.5 * pred["scada_ws_p90"]
    else:
...
def build_variant_weather(variant, weather, scada_by_name):
    ...
    by_group[GROUP3] = blend_weather(f"group3_{variant}_power_weighted_mix", by_group[GROUP3], g3_vestas, 0.30)
    return {
...
def main():
    ...
    parser.add_argument("--variants", default="pw_mean,pw_cubic,mix_pw_cubic_p90")
