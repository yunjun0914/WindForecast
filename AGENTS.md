...
이 브랜치는 모델, 데이터, 실행기, 산출물을 하나의 일관된 체계로 관리하기 위한 브랜치다.

- 모든 모델의 학습 및 예측 단위를 통일한다. Group 실험이면 PINN/TREE/TCN 모두 Group을 직접 예측하고, 터빈 실험이면 세 모델 모두 터빈을 직접 예측한다.
- Group 실험에서 터빈별 예측을 만든 뒤 합산하거나, 일부 모델만 터빈 단위를 사용하는 혼합 구조를 만들지 않는다.
- PINN/TREE/TCN은 동일한 기본 데이터, feature 계약, fold 분할을 공유한다. 모델 특성에 필요한 변환과 보조 입력만 명시적으로 분리한다.
- Group과 터빈 체계를 전환할 때는 한 모델만 바꾸지 않고 데이터 구성, 학습기, 검증, 앙상블, 추론을 같은 단위로 함께 전환한다.
- 공통 데이터 구성과 평가 로직은 재사용하고, 모델마다 별도 복사한 실행기나 중복 산출물을 만들지 않는다.
- 예측 단위나 공통 데이터 계약을 변경하기 전에는 사용자에게 구조와 영향을 먼저 설명하고 승인을 받는다.
