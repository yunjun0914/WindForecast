...
def add_weather_time_features(df, base_cols=TIME_CONTEXT_BASE_COLS):
    ...
    for col in base_cols:
        ...
        roll3 = current.rolling(window=3, min_periods=1)
        roll6 = current.rolling(window=6, min_periods=1)

        out[f"{col}_lag1"] = lag1
        out[f"{col}_lag3"] = lag3
        out[f"{col}_lead1"] = lead1
        out[f"{col}_lead3"] = lead3
        out[f"{col}_roll3_mean"] = roll3.mean()
        out[f"{col}_roll3_std"] = roll3.std(ddof=0).replace(np.nan, 0.0)
        out[f"{col}_roll6_mean"] = roll6.mean()
        out[f"{col}_roll6_std"] = roll6.std(ddof=0).replace(np.nan, 0.0)
        out[f"{col}_roll6_max"] = roll6.max()
