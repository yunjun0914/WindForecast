...
X_tensor_train, X_tensor_valid, y_tensor_train, y_tensor_valid, X_tensor_test = to_tensor.to_tensor(X_train, X_valid, target_train, target_valid, tests)
X_tensor_train = X_tensor_train[:, 1, :, :].squeeze(1)
...
# %%
from . import 경주model_test as 경주Model
...
model = 경주Model.FullModel(X_tensor_train)
...
# %%
import torch
def make_baseline(df, window_size=31):
  df = df.copy()
  df = df.fillna(0)
  ...
  mean = df['pred'].mean()
  doy_bias = torch.zeros(366)
  doy_bias[1:] = torch.tensor(df_doy['pred'].values, dtype=torch.float) - mean
  hour_bias = torch.zeros(24)
  hour_bias[:] = torch.tensor(df_hour['pred'].values, dtype=torch.float) - mean
...
gj_bs = make_baseline(pred, window_size= 7)


# %%
plt.figure(figsize= (12,4))
plt.plot(gj_bs.index, gj_bs["pred"], label = "pred")
plt.plot(gj_bs.index, gj_bs["baseline"], label = "baseline", alpha = 0.8)
#plt.plot(target2023.index, target2023, label = "real", color = "b", alpha = 0.5)
plt.legend()
plt.show()

# %%
nmae2 = nmae(target2023, gj_bs["baseline"])
print(nmae2)
